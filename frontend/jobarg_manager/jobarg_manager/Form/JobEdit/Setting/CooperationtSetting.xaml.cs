...
namespace jp.co.ftf.jobcontroller.JobController.Form.JobEdit
{
    ...
    public partial class CooperationSetting : Window
    {
        ...
        // ホストグループデータ取得 特権管理者用
        private string _selectForHostGroupSqlSuper = "select groups.groupid, groups.name as group_name, '3' as permission from groups order by groups.groupid ";
        ...
        private string _selectForHostGroupSql = "select distinct groups.groupid, groups.name as group_name, rights.permission " +
            ...
            "where users.alias = ? and rights.permission >= 0 " +
            // 2014/06/25 PostgreSQL Err対応
            "order by groups.groupid  ";
//            "order by hosts_groups.groupid  ";

        // ホストデータ取得 一般ユーザー用 ZabbixVer1.8
        private string _selectForHostSqlZabbixVer1_8 = "select users.userid, users.alias, rights.id, groups.groupid, groups.name  as group_name, " +
        ...
        private string _selectForHostSqlZabbixVer2_0 = "select groups.groupid, groups.name  as group_name, " +
        "hosts_groups.hostid, hosts.host, rights.permission from users inner join users_groups on users.userid = users_groups.userid " +
        "inner join rights on users_groups.usrgrpid = rights.groupid inner join groups on rights.id = groups.groupid " +
        "inner join hosts_groups on groups.groupid = hosts_groups.groupid inner join hosts on hosts_groups.hostid = hosts.hostid " +
            //added by YAMA 2014/08/08    （ホスト名でソート）
        ...
        private string _selectForHostSqlZabbixVer2_2 = "select users.userid, users.alias, rights.id, groups.groupid, groups.name  as group_name, " +
            "hosts_groups.hostid, hosts.host, rights.permission from users inner join users_groups on users.userid = users_groups.userid " +
            "inner join rights on users_groups.usrgrpid = rights.groupid inner join groups on rights.id = groups.groupid " +
            "inner join hosts_groups on groups.groupid = hosts_groups.groupid inner join hosts on hosts_groups.hostid = hosts.hostid " +
